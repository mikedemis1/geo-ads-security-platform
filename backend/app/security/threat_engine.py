...
Every event goes through both detectors so they can be compared.
The question this answers: which detection method is more effective
...
class SecurityAlert:
    ...
    detection_time_ms: float  # time the detector took, in ms
    event_count: int = 0
    active: bool = True
...
class RuleBasedDetector:
    """
    Threshold rules for known attack patterns.
    Each rule counts events per source IP inside a time window.
    """

    # Order matters. check() returns the first rule that fires, and both of
    # the first two rules watch auth_failed. Credential stuffing (many
    # different usernames) is the more specific pattern, so it is checked
    # before brute force (many attempts, any usernames).
    #
    # With the order reversed, stuffing was hidden whenever brute force also
    # applied, which is the usual case: ten logins from one address inside ten
    # minutes almost always puts five inside some five-minute window. It was
    # not unreachable — a slow attacker whose recent rate had dropped below
    # five was still reported correctly — but the classification depended on
    # timing rather than on the pattern. Both cases are pinned by tests.
    RULES = {
        "credential_stuffing": {
            ...
            "unique_usernames": True,  # counted over distinct usernames, not attempts
        },
        "brute_force": {
            "event_type": "auth_failed",
            "threshold": 5,
    ...
    def check(self, events: list[SecurityEvent], new_event: SecurityEvent) -> Optional[SecurityAlert]:
        """
        Check whether the new event triggers any rule.
        Returns a SecurityAlert or None.
        """
        ...
        for rule_name, rule in self.RULES.items():
            ...
            # Credential stuffing counts distinct usernames, not attempts
...
class StatisticalDetector:
    """
    Z-score anomaly detection over sliding windows.
    Alert condition: current_rate > mean + 2 standard deviations (configurable).

    Windows: 5min, 15min, 1h
    Baseline: built from past rates, refreshed periodically.
    """
    ...
    def __init__(self):
        # Rate history per window, used as the baseline
        ...
        self._baseline_interval: float = 60  # refresh the baseline every 60s

    def check(self, events: list[SecurityEvent], new_event: SecurityEvent) -> Optional[SecurityAlert]:
        """
        Z-score anomaly detection.
        Count the current rate per window and compare it with the baseline.
        """
        t0 = time.perf_counter()
        now = time.time()

        # Only suspicious event types are scored
        ...
        for window_name, window_sec in self.WINDOWS.items():
            ...
            # At least 3 data points are needed before scoring
            if len(history) < 3:
                history.append(current_rate)
                continue
...
class ThreatEngine:
    """
    The central threat detection engine.
    In-memory event and alert store with TTL-based cleanup.
    Every event goes through both detectors (rule-based and statistical)
    so the two can be compared, which was the thesis question.
    """

    EVENT_TTL = 3600  # one hour
    MAX_EVENTS = 10000
    MAX_ALERTS = 500
    ...
    def record_event(
        ...
        """
        Record a security event and run it through both detectors.
        Thread-safe.
        """
        event = SecurityEvent(
    ...
    def _add_alert(self, alert: SecurityAlert) -> None:
        """Store an alert and push it to connected WebSocket clients."""
    ...
    def _cleanup_old_events(self) -> None:
        """Drop events older than EVENT_TTL."""
    ...
    def get_comparison(self) -> dict:
        """Return the rule-based vs statistical comparison data."""
        with self._lock:
